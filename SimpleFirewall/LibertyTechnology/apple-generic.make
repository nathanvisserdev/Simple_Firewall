...
ifeq "" "$(CURRENT_PROJECT_VERSION)"
	...
	$(SILENT) printf "%s%s\n" $(VERSION_INFO_EXPORT_DECL) " const unsigned char $(VERSION_INFO_PREFIX)$(NAME)VersionString$(VERSION_INFO_SUFFIX)[] = \"@(#)PROGRAM:$(NAME)  PROJECT:$(NAME)-$(LOCAL_CURRENT_PROJECT_VERSION)  DEVELOPER:$(VERSION_INFO_BUILDER)  BUILT:\" __DATE__  \" \" __TIME__ \"\n\";" > $(VERS_FILE)
	$(SILENT) printf "%s%s\n" $(VERSION_INFO_EXPORT_DECL) " const double $(VERSION_INFO_PREFIX)$(NAME)VersionNumber$(VERSION_INFO_SUFFIX) = (double)$(LOCAL_CURRENT_PROJECT_VERSION);" >> $(VERS_FILE)
